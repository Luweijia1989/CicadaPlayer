...
public:
    ...
    static inline float dotProduct(const QQuaternion &q1, const QQuaternion &q2);
...
inline float QQuaternion::dotProduct(const QQuaternion &q1, const QQuaternion &q2)
{
    return q1.wp * q2.wp + q1.xp * q2.xp + q1.yp * q2.yp + q1.zp * q2.zp;
}
...
inline QQuaternion QQuaternion::inverted() const
{
    ...
    double len = double(wp) * double(wp) + double(xp) * double(xp) + double(yp) * double(yp) + double(zp) * double(zp);
    if (!qFuzzyIsNull(len))
        return QQuaternion(float(double(wp) / len), float(double(-xp) / len), float(double(-yp) / len), float(double(-zp) / len));
...
{
    return qFuzzyCompare(q1.wp, q2.wp) && qFuzzyCompare(q1.xp, q2.xp) && qFuzzyCompare(q1.yp, q2.yp) && qFuzzyCompare(q1.zp, q2.zp);
}

inline QQuaternion::QQuaternion(float aScalar, const QVector3D &aVector) : wp(aScalar), xp(aVector.x()), yp(aVector.y()), zp(aVector.z())
{}
...
inline QQuaternion::QQuaternion(const QVector4D &aVector) : wp(aVector.w()), xp(aVector.x()), yp(aVector.y()), zp(aVector.z())
{}
