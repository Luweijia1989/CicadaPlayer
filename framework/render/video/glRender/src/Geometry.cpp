...
Attribute::Attribute(DataType type, int tupleSize, int offset, bool normalize)
    : m_normalize(normalize), m_type(type), m_tupleSize(tupleSize), m_offset(offset)
{}

Attribute::Attribute(const std::string &name, DataType type, int tupleSize, int offset, bool normalize)
    : m_normalize(normalize), m_type(type), m_tupleSize(tupleSize), m_offset(offset), m_name(name)
{}

Geometry::Geometry(int vertexCount, int indexCount, DataType indexType)
    : m_primitive(TriangleStrip), m_itype(indexType), m_vcount(vertexCount), m_icount(indexCount)
{}
...
TexturedGeometry::TexturedGeometry() : Geometry(), nb_tex(0), geo_rect(-1, 1, 2, -2)// (-1, -1, 2, 2) flip y
{
    setVertexCount(4);
    a = {Attribute(TypeF32, 2, 0), Attribute(TypeF32, 2, 2 * sizeof(float))};
    setTextureCount(1);
}

void TexturedGeometry::setTextureCount(int value)
{
    if (value == nb_tex) return;
...
{
    if (texRect.size() <= texIndex) texRect.resize(texIndex + 1);
    texRect[texIndex] = tr;
}
...
void TexturedGeometry::create()
{
    ...
    if (a.size() - 1 < textureCount()) {// the first is position
        for (int i = a.size() - 1; i < textureCount(); ++i) a.push_back(Attribute(TypeF32, 2, int((i + 1) * 2 * sizeof(float))));
    } else {
...
Sphere::Sphere() : TexturedGeometry(), r(1)
{
    setPrimitive(Triangles);
    setResolution(128, 128);
    a = {Attribute(TypeF32, 3, 0), Attribute(TypeF32, 2, 3 * sizeof(float))};
}
...
void Sphere::create()
{
    ...
    if (a.size() - 1 < nb_tex) {             // the first is position
        for (int i = a.size() - 1; i < nb_tex; ++i) a.push_back(Attribute(TypeF32, 2, 3 * sizeof(float) + int(i * 2 * sizeof(float))));
    } else {
        a.resize(nb_tex + 1);
    }
