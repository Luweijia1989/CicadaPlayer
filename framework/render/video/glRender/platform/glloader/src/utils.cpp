...
namespace
{
	class LoadHelper {
	public:
		~LoadHelper() { glloader_uninit(); }
	};

	static LoadHelper g_glLoaderHelper;
	...
	typedef PROC (WINAPI *wglGetCurrentContextFUNC)();
    wglGetCurrentContextFUNC DynamicWglGetCurrentContext;
#endif
}
...
namespace glloader
{
	...
	private:
		gl_features_extractor()
		{
            ...
            DynamicWglGetCurrentContext = (wglGetCurrentContextFUNC)(glloader_get_gl_proc_address("wglGetCurrentContext"));
#endif
...
void *glloader_current_gl_ctx()
{
	glloader_init();
#ifdef GLLOADER_WGL
	if (!DynamicWglGetCurrentContext)
		return NULL;
    else
