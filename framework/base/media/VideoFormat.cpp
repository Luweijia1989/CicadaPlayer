...
public:
    VideoFormatPrivate(VideoFormat::PixelFormat fmt)
        : pixfmt(fmt), pixfmt_ff(QTAV_PIX_FMT_C(NONE)), planes(0), bpp(0), bpp_pad(0), bpc(0), pixdesc(0)
    {
    ...
    VideoFormatPrivate(AVPixelFormat fmt)
        : pixfmt(VideoFormat::Format_Invalid), pixfmt_ff(fmt), planes(0), bpp(0), bpp_pad(0), bpc(0), pixdesc(0)
    {
        init(fmt);
    }
    ...
    void init()
    {
        ...
        if (pixfmt_ff == QTAV_PIX_FMT_C(NONE)) {
            AF_LOGW("Invalid pixel format");
            return;
        }
        planes = std::max<unsigned char>(av_pix_fmt_count_planes(pixfmt_ff), 0);
        bpps.reserve(planes);
    ...
    {
        return std::string(av_get_pix_fmt_name(pixfmt_ff));
    }
    int flags() const
    {
        if (!pixdesc) return 0;
...
private:
    ...
    void initBpp()
    {
        ...
        for (int c = 0; c < pixdesc->nb_components; c++) {
            ...
            bpps[comp->plane] += comp->depth;
            steps[comp->plane] = comp->step << s;
            channels[comp->plane] += 1;
            bpp += comp->depth << s;
            if (comp->depth != bpc) bpc = 0;
        }
        for (int i = 0; i < planes; ++i) {
            bpp_pad += steps[i];
        }
        if (!(pixdesc->flags & AV_PIX_FMT_FLAG_BITSTREAM)) bpp_pad *= 8;
        bpp >>= log2_pixels;
        bpp_pad >>= log2_pixels;
    }
};
...
static const struct {
    ...
    AVPixelFormat ff;//int
        ...
        {VideoFormat::Format_YUV420P,
         QTAV_PIX_FMT_C(
                 YUVJ420P)},///< planar YUV 4:2:0, 12bpp, full scale (JPEG), deprecated in favor of PIX_FMT_YUV420P and setting color_range
    ...
    */
        ...
        {VideoFormat::Format_RGB555,
         QTAV_PIX_FMT_C(RGB555)},///< packed RGB 5:5:5, 16bpp, (msb)1A 5R 5G 5B(lsb), native-endian, be: most significant bit to 1
        {VideoFormat::Format_BGR565, QTAV_PIX_FMT_C(BGR565)},///< packed BGR 5:6:5, 16bpp, (msb)   5B 6G 5R(lsb), native-endian
        {VideoFormat::Format_BGR555,
         QTAV_PIX_FMT_C(BGR555)},///< packed BGR 5:5:5, 16bpp, (msb)1A 5B 5G 5R(lsb), native-endian, be: most significant bit to 1
                                 /*
...
*/
        ...
        {VideoFormat::Format_RGB48,
         QTAV_PIX_FMT_C(
                 RGB48)},///< packed RGB 16:16:16, 48bpp, 16R, 16G, 16B, the 2-byte value for each R/G/B component is stored as big-endian
        {VideoFormat::Format_BGR48,
         QTAV_PIX_FMT_C(
                 BGR48)},///< packed RGB 16:16:16, 48bpp, 16B, 16G, 16R, the 2-byte value for each R/G/B component is stored as big-endian
...
VideoFormat::PixelFormat VideoFormat::pixelFormatFromFFmpeg(int ff)
{
    for (unsigned int i = 0; i < sizeof(pixfmt_map) / sizeof(pixfmt_map[0]); ++i) {
        if (pixfmt_map[i].ff == ff) return pixfmt_map[i].fmt;
    }
...
{
    for (unsigned int i = 0; i < sizeof(pixfmt_map) / sizeof(pixfmt_map[0]); ++i) {
        if (pixfmt_map[i].fmt == fmt) return pixfmt_map[i].ff;
    }
...
{
    static std::vector<int> sFmts;
    if (sFmts.size() == 0) {
        const AVPixFmtDescriptor *desc = NULL;
        while ((desc = av_pix_fmt_desc_next(desc))) {
            if ((desc->flags & AV_PIX_FMT_FLAG_HWACCEL) == AV_PIX_FMT_FLAG_HWACCEL) continue;
            sFmts.push_back(av_pix_fmt_desc_get_id(desc));
        }
    }
    return sFmts;
}
...
VideoFormat::VideoFormat(PixelFormat format) : d(new VideoFormatPrivate(format))
{}

VideoFormat::VideoFormat(int formatFF) : d(new VideoFormatPrivate((AVPixelFormat) formatFF))
{}

VideoFormat::VideoFormat(const std::string &name) : d(new VideoFormatPrivate(av_get_pix_fmt(name.c_str())))
{}

VideoFormat::VideoFormat(const VideoFormat &other) : d(new VideoFormatPrivate(other.d->pixfmt))
{}
...
{
    d = std::make_unique<VideoFormatPrivate>(other.d->pixfmt);
    return *this;
}

VideoFormat &VideoFormat::operator=(VideoFormat::PixelFormat fmt)
{
    d = std::make_unique<VideoFormatPrivate>(fmt);
    return *this;
}

VideoFormat &VideoFormat::operator=(int fffmt)
{
    d = std::make_unique<VideoFormatPrivate>((AVPixelFormat) fffmt);
    return *this;
}
...
{
    if (plane <= 0) return lumaWidth;
    return chromaWidth(lumaWidth);
}

int VideoFormat::height(int lumaHeight, int plane) const
{
    if (plane <= 0) return lumaHeight;
    return chromaHeight(lumaHeight);
}

double VideoFormat::normalizedWidth(int plane) const
{
    if (plane <= 0) return 1.0;
    return 1.0 / std::pow(2.0, double(d->pixdesc->log2_chroma_w));
}

double VideoFormat::normalizedHeight(int plane) const
{
    if (plane <= 0) return 1.0;
    return 1.0 / std::pow(2.0, double(d->pixdesc->log2_chroma_h));
}
...
bool VideoFormat::isPlanar(PixelFormat pixfmt)
{
    return pixfmt == Format_YUV420P || pixfmt == Format_NV12 || pixfmt == Format_NV21 || pixfmt == Format_YV12 ||
           pixfmt == Format_YUV410P || pixfmt == Format_YUV411P || pixfmt == Format_YUV422P || pixfmt == Format_YUV444P ||
           pixfmt == Format_AYUV444 || pixfmt == Format_IMC1 || pixfmt == Format_IMC2 || pixfmt == Format_IMC3 || pixfmt == Format_IMC4;
}

bool VideoFormat::isRGB(PixelFormat pixfmt)
{
    return pixfmt == Format_RGB32 || pixfmt == Format_ARGB32 || pixfmt == Format_RGB24 || pixfmt == Format_BGRA32 ||
           pixfmt == Format_ABGR32 || pixfmt == Format_RGBA32 || pixfmt == Format_BGR565 || pixfmt == Format_RGB555 ||
           pixfmt == Format_RGB565 || pixfmt == Format_BGR24 || pixfmt == Format_BGR32 || pixfmt == Format_BGR555 ||
           pixfmt == Format_RGB48 || pixfmt == Format_RGB48LE || pixfmt == Format_RGB48BE || pixfmt == Format_BGR48 ||
           pixfmt == Format_BGR48LE || pixfmt == Format_BGR48BE || pixfmt == Format_RGBA64 || pixfmt == Format_RGBA64LE ||
           pixfmt == Format_RGBA64BE || pixfmt == Format_BGRA64 || pixfmt == Format_BGRA64LE || pixfmt == Format_BGRA64BE;
}
