...
AttrsNode::AttrsNode(Type t, AttrsNode *parent_) : AbstractAttr(t)
{
...
void AttrsNode::addAttribute(AbstractAttr *attr)
{
    props.push_front(attr);
    attr->setParentNode(this);
}

void AttrsNode::replaceAttribute(AbstractAttr *attr)
{
    AbstractAttr *old = getAttribute(attr->getType());
    if (old) {
        props.remove(old);
        delete old;
    }
...
int64_t AttrsNode::inheritDuration() const
{
    const AbstractAttr *p = inheritAttribute(Type::Duration);
    if (p && p->isValid()) {
        return (const int64_t &) *(static_cast<const DurationAttr *>(p));
    }
    return 0;
}
...
int64_t AttrsNode::inheritAvailabilityTimeOffset() const
{
    const AbstractAttr *p = inheritAttribute(Type::AvailabilityTimeOffset);
    if (p && p->isValid()) {
        return (const int64_t &) *(static_cast<const AvailabilityTimeOffsetAttr *>(p));
    }
    return 0;
}
...
AbstractAttr *AttrsNode::getAttribute(AbstractAttr::Type type, std::list<AbstractAttr::Type> &path)
{
