...
static const dxgi_format_t dxgi_formats[] = {{"NV12", DXGI_FORMAT_NV12, VLC_CODEC_NV12},
...
{
    for (const dxgi_format_t *f = dxgi_formats; f->name != NULL; ++f) {
        if (f->format == format) return f->vlc_format;
    }
    return 0;
}
...
const char *DxgiVendorStr(int gpu_vendor)
{
    static const struct {
        unsigned id;
        const char name[32];
    } vendors[] = {{GPU_MANUFACTURER_AMD, "ATI"},
    ...
    for (i = 0; vendors[i].id != 0; i++) {
        if (vendors[i].id == gpu_vendor) break;
    }
...
UINT DxgiResourceCount(const d3d_format_t *d3d_fmt)
{
    for (UINT count = 0; count < D3D11_MAX_SHADER_VIEW; count++) {
        if (d3d_fmt->resourceFormat[count] == DXGI_FORMAT_UNKNOWN) return count;
    }
