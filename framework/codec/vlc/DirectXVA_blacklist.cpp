...
static struct {
    ...
    const struct decoders *devices;
        ...
        {GPU_MANUFACTURER_INTEL, IntelDevices},
};

bool DirectXVA::directxVAcanUseDecoder(UINT VendorId, UINT DeviceId, const GUID *pCodec, UINT driverBuild)
{
    for (size_t i = 0; i < (sizeof(gpu_blacklist) / sizeof((gpu_blacklist)[0])); i++) {
                ...
                if (pDevice->deviceID == DeviceId) {
                    ...
                    while (*pGuid != NULL) {
                        if (IsEqualGUID(*pCodec, **pGuid)) {
                            if (pDevice->cmd == BLAnyDriver) return false;
                            if (pDevice->cmd == BLBelowBuild && driverBuild < pDevice->driverBuild) return false;
                            break;
                        }
                        pGuid++;
                    }
